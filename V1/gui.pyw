...
class TimerApp:
        ...
        for proc in psutil.process_iter(['pid', 'name']):
            try:
                ...
                if proc.info['name'] == name:
                    ...
                    p.kill()  # Attempt to terminate the process
                    p.wait(timeout=5)  # Wait for process to terminate
            except (psutil.NoSuchProcess, psutil.AccessDenied) as e:print(f"Error handling process {proc.info['pid']}: {e}")
    def start_timer(self):
        self.is_running = True
        self.start_button.config(state=tk.DISABLED)
        self.stop_button.config(state=tk.NORMAL)

        selected_time = f"{self.hour_var.get():02}:{self.minute_var.get():02} {self.ampm_var.get()}"
        self.show_custom_messagebox("Timer Started", f"Timer started for {selected_time}.")
        self.stop_event.clear()
        
        # Create and start a new thread
        self.timer_thread = threading.Thread(target=self.run_timer, args=(selected_time,))
        print("starting thread")
        self.timer_thread.start()
    def run_timer(self, sleep_time):
        """Run the timer by starting the executable."""
        try:
            print("starting timer")
            # Start the external executable with the sleep time as an argument
            if self.CustomTimerPath is not None:self.process=subprocess.Popen([self.CustomTimerPath, str(sleep_time)],
                                                stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            else:self.process = subprocess.Popen(['sleep_timer.exe', str(sleep_time)],
                                                stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            while True:
                if self.stop_event.is_set():
                    print("Timer stopped.")
                    self.kill_processes_by_name("sleep_timer.exe")
                if self.process.poll() is not None:  # Check if the process has terminated
                    print("Timer completed.")
                    return
                
                time.sleep(1)
        except Exception as e:print(f"Timer thread encountered an exception: {e}")
        finally:
            if self.stop_event.is_set():
                # Ensure the executable is stopped if the timer is cancelled
    ...
    def stop_timer(self):
        if self.is_running:
            ...
            self.show_custom_messagebox("Timer Stopped", "Timer has been stopped.")
            if self.timer_thread is not None:
                self.stop_event.set()  # Signal the timer thread to stop
                self.timer_thread.join()  # Wait for the thread to finish
            if self.process is not None:
                self.process.terminate()  # Ensure the executable is stopped
                self.process.wait()  # Ensure the process has been terminated
...
if __name__ == "__main__":
    ...
    try:
        if isfile(argv[1]):
            if argv[1].__contains__("sleep_timer"):app = TimerApp(root,argv[1])
            app=TimerApp(root)
        else:app=TimerApp(root)
    except IndexError:
        app=TimerApp(root)
    root.mainloop()
